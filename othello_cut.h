...
class state_t {
  ...
  public:
    ...
    std::list<int> succ(bool color) const;
    std::list<state_t> succe(bool color) const;

    state_t genInit(int depth, bool &color) const;
};
...
inline std::list<int> state_t::succ(bool color) const{
    std::list<int> l;
    for ( int i = 4 ; i < DIM ; ++i )
        if (outflank(color,i))
            l.push_back(i);
...
inline std::list<state_t> state_t::succe(bool color) const{
    std::list<state_t> l;

    for ( int i = 4 ; i < DIM ; ++i )
        if (outflank(color,i))
            l.push_back(move(color,i));
    return l;
}

inline state_t state_t::genInit(int depth, bool &color) const{

    state_t result;
    color = true;

    for(int i = 0; i < depth; i++, color = !color)
        result = result.move(color,PV[i]);
